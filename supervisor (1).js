...
async function initAuthThenRender() {
  ...
  if (ok) {
    ...
    // Login gate is currently non-functional (no /api/verify endpoint,
    // no showLoginModal implementation) — disarmed rather than deleted,
    // since the real page render doesn't depend on this succeeding.
    console.warn('Auth check failed or /api/verify is unavailable — skipping login modal (not implemented).');
  }
}
...
// Shared by every path that changes the totals — recomputes the fill width
// and full/near-full color state from whatever's currently in the two
// count spans, so the bar never goes stale no matter which code path fired.
function refreshCapacityBar() {
  const sEl = document.getElementById('total-students-count');
  const cEl = document.getElementById('total-capacity-count');
  const fillEl = document.getElementById('capacity-bar-fill');
  const barEl = document.getElementById('capacity-bar');
  if (!sEl || !cEl || !fillEl || !barEl) return;
  const students = parseInt(sEl.textContent || '0', 10) || 0;
  const cap = parseInt(cEl.textContent || '0', 10) || 0;
  const pct = cap > 0 ? Math.min(100, Math.round((students / cap) * 100)) : 0;
  fillEl.style.width = pct + '%';
  barEl.classList.remove('is-near-full', 'is-full');
  if (cap > 0 && students >= cap) barEl.classList.add('is-full');
  else if (cap > 0 && students >= cap - Math.max(1, Math.round(cap * 0.1))) barEl.classList.add('is-near-full');
}
...
async function updateRoomCountsInPlace() {
  try {
    ...
    rowCache.forEach((row, keyLC) => {
      ...
      applyRoomRowColor(row, null); // room object not in scope here — reads active/hex from the row itself
    });
...
// Picks readable text color against a given room background
function textForRoomBg(hex) {
  try {
    const c = hex.replace('#', '');
...
// Replaces the old full/near-full/closed capacity-based row tinting.
// Active rooms get their own color as the row background; inactive rooms
// keep the plain "closed" look. Reads active/hex from data-attributes so
// callers that only have a rowEl (not the full room object) still work.
function applyRoomRowColor(rowEl, room) {
  rowEl.classList.remove('full-room', 'near-full-room', 'closed-room', 'room-row--active');
  const isActive = room && typeof room.active !== 'undefined'
    ? room.active !== false
    : rowEl.dataset.roomActive !== 'false';

  // Capacity 0 means "closed for now" regardless of the active flag — prefer
  // an explicit room.capacity (available on initial build) and fall back to
  // reading the row's own live input (used by every update-in-place path).
  let cap;
  if (room && room.capacity !== undefined && room.capacity !== null) {
    cap = room.capacity;
  } else {
    const capInput = rowEl.querySelector('input[type="number"]');
    cap = capInput ? (parseInt(capInput.value, 10) || 0) : 0;
  }

  if (!isActive || cap === 0) {
    rowEl.dataset.roomActive = isActive ? 'true' : 'false';
    rowEl.style.backgroundColor = '';
    rowEl.style.color = '';
    rowEl.classList.add('closed-room');
    return;
  }
  rowEl.dataset.roomActive = 'true';

  const applied = room && (room.color_hex || room.icon_emoji) ? computeRoomStyle(room) : null;
  let hex = applied?.color_hex ? String(applied.color_hex).trim() : (rowEl.dataset.roomHex || '');
  if (hex && !hex.startsWith('#')) hex = `#${hex}`;

  if (hex && /^#[0-9a-fA-F]{6}$/.test(hex)) {
    rowEl.dataset.roomHex = hex;
    rowEl.classList.add('room-row--active');
    rowEl.style.backgroundColor = hex;
    rowEl.style.color = textForRoomBg(hex);
  } else {
    rowEl.style.backgroundColor = '';
    rowEl.style.color = '';
  }
...
function placeSiteDropdownInChip() {
  ...
  // Search the whole container directly instead of assuming a specific
  // wrapper depth — this was the actual bug: it was looking one level too
  // deep, inside .summary-card__top for another .summary-card__top.
  const card = sessionContainer ? sessionContainer.querySelector('.summary-card__top') : null;
  ...
  const wrap = siteSelect.closest('.site-field') || siteSelect.closest('label') || siteSelect;
...
async function clearCapacitiesFor(site, timeSlot /* can be null */) {
  ...
  try {
    ...
    validRoomNames.forEach((name) => {
      ...
      // Capacity cleared to 0, but the room itself is still active —
      // keep its own color rather than forcing the closed look
      applyRoomRowColor(rowEl, null);
    });
...
async function renderSession(sessionKey) {
    const myRenderToken = ++renderSessionToken;
    ...
    if (myRenderToken !== renderSessionToken) return; // a newer render already started — abandon this one before touching the DOM at all

    if (studentsError) {
      console.error("Error fetching students for session render:", studentsError);
      detachSiteDropdownFromChip(); // park it safely before this branch wipes the container
    ...
    if (myRenderToken !== renderSessionToken) return;

    // Preserve the live site dropdown before rewriting the chip
    detachSiteDropdownFromChip();
    const pct = totalCapacity > 0 ? Math.min(100, Math.round((totalStudentsAssigned / totalCapacity) * 100)) : 0;
    const html = `
              <div class="summary-card__top">
          <label class="site-field">
            <span class="site-field__icon">🕐</span>
            <span class="site-field__body">
              <span class="site-field__label">Start Time</span>
        ...
        <div class="capacity-bar" id="capacity-bar">
          <div class="capacity-bar__track">
            <div class="capacity-bar__fill" id="capacity-bar-fill" style="width:${pct}%;"></div>
          </div>
          <div class="capacity-bar__label">
            <span id="total-students-count" class="capacity-bar__count">${totalStudentsAssigned}</span>
            <span class="capacity-bar__of">of</span>
            <span id="total-capacity-count" class="capacity-bar__count capacity-bar__count--max">${totalCapacity}</span>
            <span class="capacity-bar__caption">students placed</span>
          </div>
        </div>
      </div>
    `;
  ...
  // Safety net only: realtime + in-memory deltas (applyRosterDelta) handle
  // accuracy instantly now. This slow interval just self-corrects if a
  // browser tab ever misses a realtime message — it is not the primary
  // update path anymore.
...
// Self-healing: whenever #session-container's contents change for any
// reason, make sure the site dropdown ends up back in the right spot.
// This sidesteps needing to track down every code path that might
// rebuild the container — instead of chasing each one individually,
// this guarantees correctness no matter what caused the change.
document.addEventListener('DOMContentLoaded', () => {
