...
async function fetchFilteredStudents() {
  ...
  const build = (selectCols) => {
   ...
   // Confirmed schema: the column is "kgroup" (singular) — no need to guess anymore.
 let query = build('id, firstname, lastname, grade, kgroup');
...
function applyDeviceFilterRow(row) {
  ...
  updateSiteTitleFromDevice();
   ...
   if (filterChanged) {
     startKioskRealtime(); // reassigned to a different roster — rebuild the scoped channel
     loadStudents();
     loadRoomStatusBar();
   }
}
...
// Realtime: listen to master_roster & rooms, scoped to this device's own
// site/summer_site/non_school_day list. Previously unfiltered — every
// kiosk received every room change at every site.
let __kioskRealtimeChannel = null;

function buildKioskRosterFilter() {
  const col = currentDevice?.column;
  const val = currentDevice?.value;
  if (!col) return null;
  if (col === 'non_school_day') return `non_school_day=eq.true`;
  const safeVal = String(val).replace(/,/g, '\\,');
  return `${col}=eq.${safeVal}`;
}

function startKioskRealtime() {
  if (__kioskRealtimeChannel) {
    try { supabase.removeChannel(__kioskRealtimeChannel); } catch {}
    __kioskRealtimeChannel = null;
  }

  const rosterFilter = buildKioskRosterFilter();
  const roomsSite = resolveSiteForRoomsFromDevice();
  if (!rosterFilter || !roomsSite) return; // device not configured yet

  const rosterCfg = { event: '*', schema: 'public', table: 'master_roster', filter: rosterFilter };
  const roomsCfg = { event: '*', schema: 'public', table: 'rooms', filter: `site=eq.${roomsSite}` };

  __kioskRealtimeChannel = supabase
    .channel('public:student_ui')
    .on('postgres_changes', rosterCfg, (payload) => {
...
async function loadStudents() {
  ...
  uniqueGradeKeys.forEach((gradeKey, index) => {
    ...
    if (isAlbertvillePrimary && gradeKey === 'K') {
      ...
      const groupOf = (s) => s.kgroup ?? 'Group';
...
function renderEligibleRooms(rooms, counts, student, site, timeSlot) {
  ...
  if (site === 'Club Knights') {
    ...
    activityBtn.addEventListener('click', () => markStudentActivityInBuilding(student.id, site));
  ...
  goneBtn.addEventListener('click', () => markStudentGone(student.id, site));
...
async function chooseRoom(studentId, site, roomName, timeSlot) {
  ...
  try {
    ...
    if (data === 'room_restricted' || data === 'student_restricted') {
      showMessage(`Sorry, ${roomName} isn't available for you right now. Please choose another room.`, false);
    } else if (data === 'room_full') {
...
async function markStudentGone(studentId, site) {
  try {
    const { data, error } = await supabase.rpc('mark_student_gone_v2', {
      p_student_id: Number(studentId),
      p_site: site
    });
    if (error) throw error;
...
async function markStudentActivityInBuilding(studentId, site) {
  try {
    const { data, error } = await supabase.rpc('mark_student_activity_in_building_v2', {
      p_student_id: Number(studentId),
      p_site: site
    });
    if (error) throw error;
...
(async () => {
  ...
  updateSiteTitleFromDevice();
  startKioskRealtime(); // currentDevice is known now — scope the channel to this site
  await loadSessionTimes();
